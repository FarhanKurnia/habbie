<div class="dropdown dropdown-end z-10">
    <label tabindex="0" class="btn btn-ghost btn-circle lg:bg-grey-secondary-50">
        <div class="indicator">
            ...
            <span class="badge badge-sm indicator-item h-4 w-4 p-2 bg-pink-primary text-white">{{ Cart::getTotalQuantity() }} </span>
        </div>
    </label>
    <div tabindex="0" class="mt-3 card card-compact dropdown-content w-52 bg-base-100 shadow">
        <div class="card-body">
            <span class="font-bold text-lg">{{ $totalItems }} items</span>
            <span class="text-info">Subtotal: Rp{{ \App\Helpers\CurrencyFormat::data($subTotal) }}</span>
            <a href="{{ url('/cart') }}">
