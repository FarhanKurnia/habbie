...
@php
    $title = isset($oneRecommendation) ? 'Update Recommendation' : 'Create New Recommendation';
@endphp
@section('title', $title)
...
@section('content')
    <div class="mx-auto max-w-screen-2xl h-screen p-4 md:p-6 2xl:p-10 ">
        ...
        <form class="grid grid-cols-4 gap-2 my-6" method="POST" enctype="multipart/form-data"
            action="{{ isset($oneRecommendation) ? route('updateRecommendations', $oneRecommendation->slug) : route('storeRecommendations') }}">
            @if (isset($oneRecommendation))
                @csrf
                @method('PATCH')
            @else
                {{ csrf_field() }}
            ...
            <div class="col-span-3 space-y-4">
                <input type="text" name="name" class="input input-bordered w-full bg-grey-secondary-50 "
                    placeholder="Recommendation Title" required value="{{ isset($oneRecommendation) ? $oneRecommendation->name : old('name') }}">
                @error('name')
                            ...
                            <div class="rounded-lg p-4 bg-pink-primary bg-opacity-10 flex items-center">
                                <div class="w-14 px-auto">
                                    <input type="radio" class="radio checked:bg-pink-primary" name="product"
                                        ...
                                        {{ isset($oneRecommendation) && $oneRecommendation->product_id === $item->id_product ? 'checked' : '' }}
                                        required />
                                </div>
            ...
            <div class="mx-2 flex flex-col gap-4">
                <button type="submit" value="Proses"
                    class="btn btn-primary text-white ">{{ isset($oneRecommendation) ? 'Update' : 'Publish' }}</button>
                ...
                <div class="p-4 bg-white border border-grey-secondary rounded-lg">
                    <label class="cursor-pointer" for="product-image">
                        ...
                        @if (isset($oneRecommendation))
                            <img class="h-48 mx-auto m-8" src="{{ url($oneRecommendation->image) }}" alt="{{ $oneRecommendation->name }}">
                        @endif
                    </label>
                    <input type="file" id="recommendation-image" name="image"
        ...
        <script>
            ClassicEditor
                ...
                .then(editor => {
                    editor.setData('{!! isset($oneRecommendation) ? $oneRecommendation->description : '' !!}');
                })
                .catch(error => {
                    console.error(error);
                });
